...
namespace BizTalk.Adapter.DesignTime.AssemblySelectEditor
{
    public partial class AssemblyPopupForm : Form
    {
        ...
        private string assemblyFullyQualifiedName = string.Empty;
        public AssemblyPopupForm(string value)
        {
            InitializeComponent();
            this.okbutton.Enabled = false;
            assemblyFullyQualifiedName = value;
            this.listBox1.Items.Clear();

            if (!string.IsNullOrEmpty(assemblyFullyQualifiedName))
            {
                Type type = Type.GetType(assemblyFullyQualifiedName);
                if (type != null)
                {
                    ...
                    assemblyFullyQualifiedName = string.Empty;
                }
            }
        ...
        private void filllist(string assemlbyFullyQualifiedName) {
            ...
            var schemaList = (from type in _assembly.GetTypes()
                              where !type.IsAbstract && typeof(BizTalk.Adapter.AssemblyExecute.Interface.IAssemblyExecute).IsAssignableFrom(type)
                              select type.AssemblyQualifiedName).ToList();
            this.listBox1.Items.Clear();
            foreach (var str in schemaList) {
                this.listBox1.Items.Add(str);
                if (str == this.assemblyFullyQualifiedName)
                {
        ...
        private void AssemblyPopupForm_Load(object sender, EventArgs e)
        {
